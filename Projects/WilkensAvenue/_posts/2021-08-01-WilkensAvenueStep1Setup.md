...
Similar to the SafeCardGame, I am going to utilize the `SAFE Template` to scaffold the initial site. This time though before making any code changes I am going to set up the Heroku setup and database configuration along with the appropriate configuration for test and scripts to publish. Previously, I delayed that and it was quite a pain point to do after the fact. Luckily, this time I am using a newer version of the `SAFE` template which rolls with .NET 5 out of the box. In the `SafeCardGame` project I manually upgraded that which turned out to be quite a pain.
